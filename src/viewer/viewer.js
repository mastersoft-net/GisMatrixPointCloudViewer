...
/**
 * Main Viewer class for Potree.
 * Handles the initialization of the 3D scene, rendering loop, GUI components, and user interaction.
 */
export class Viewer extends EventDispatcher {

	/**
	 * Initializes the viewer within the given DOM element.
	 * 
	 * @param {HTMLElement} domElement - The HTML element that will contain the viewer.
	 * @param {Object} args - Optional configuration parameters.
	 */
	 ...
	 * Moves the render area and explicitly sets the sidebar container's visibility to hidden
	 * when closed to prevent it from bleeding through the transparent WebGL canvas.
	 */
	toggleSidebar() {
		...
		if (isVisible) {
			...
			// MODIFICATION: Wait for the slide transition (usually 300ms) to finish, 
			// then hide the sidebar to avoid ghosting through the canvas.
			setTimeout(() => {
				if (renderArea.css('left') === '0px') {
					sidebarContainer.css('visibility', 'hidden');
				}
			}, 300);
		} else {
			// MODIFICATION: Make the sidebar visible before sliding the render area.
	...
	/**
	 * Loads the graphical user interface (GUI) and initializes the sidebar.
	 * 
	 * @param {Function} [callback] - Optional callback function executed after GUI is loaded.
	 * @returns {Promise} A promise that resolves when the GUI finishes loading.
	 */
	loadGUI(callback) {
		...
		sidebarContainer.load(new URL(Potree.scriptPath + '/sidebar.html').href, () => {
			...
			// --- MODIFICATION: Hide the sidebar initially if the render area is closed (left is 0px) ---
			// This prevents the sidebar from bleeding through the transparent WebGL canvas on initial load.
			let renderArea = $('#potree_render_area');
			if (renderArea.css('left') === '0px') {
				sidebarContainer.css('visibility', 'hidden');
			}
	...
	localizeAttribute(attribute, toLower) {
		const attr = this.localizeAttribute_(attribute);

		if (attr != null) {
			return toLower ? attr.toLowerCase() : attr;
